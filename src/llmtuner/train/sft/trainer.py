    ...
    def evaluation_loop(
        ...
        for step, inputs in enumerate(dataloader):
            ...
            decoded_text = self.tokenizer.decode(
                inputs["input_ids"].tolist()[0], add_special_tokens=False
            )
            pattern = r"Question: \n(.*?)\n"
            match = re.search(pattern, decoded_text)
            result_dict = {
                # "hash_value": hash(inputs["input_ids"].detach().cpu()),
                "question": match.group(1) if match else "",
                "loss": loss.detach().item(),
            }
            with open("cherry_data_0325.json", "a", encoding="utf-8") as writer:
                json.dump(result_dict, writer, ensure_ascii=False)
                writer.write("\n")
